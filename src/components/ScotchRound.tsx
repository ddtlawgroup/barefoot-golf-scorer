...
import { PLAYERS, ROUNDS, DEFAULT_PARS } from '@/lib/types';
...
export default function ScotchRound({ round }: { round: number }) {
  const { trip, getPlayerNetScores, getPar, getHoleExtra } = useTripContext();
  ...
  const result = calcScotchRound(netScores, scotchTeams, pars, girPlayers);

  const players = (trip?.players ?? []).map(p => p.name);
  const teamNames = (indices: number[]) => indices.map(i => players[i] ?? PLAYERS[i]).join(' & ');

  // Determine current segment based on scores entered
